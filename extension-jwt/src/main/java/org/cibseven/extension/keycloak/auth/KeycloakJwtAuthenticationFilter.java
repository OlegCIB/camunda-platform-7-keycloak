package org.cibseven.extension.keycloak.auth;
...
import org.cibseven.bmp.engine.ProcessEngine;
import org.cibseven.bmp.engine.ProcessEngines;
import org.cibseven.bmp.engine.rest.security.auth.AuthenticationResult;
import org.cibseven.bmp.webapp.impl.security.SecurityActions;
import org.cibseven.bmp.webapp.impl.security.auth.Authentications;
import org.cibseven.bmp.webapp.impl.security.auth.ContainerBasedAuthenticationFilter;
import org.cibseven.bmp.webapp.impl.security.auth.UserAuthentication;
 ...
 *
 * @see org.cibseven.bpm.webapp.impl.security.auth.AuthenticationFilter
 * @see org.cibseven.bpm.webapp.impl.security.auth.ContainerBasedAuthenticationFilter
 */
public class KeycloakJwtAuthenticationFilter extends ContainerBasedAuthenticationFilter {
    ...
    private String cibsevenApplicationPath = "";

    public KeycloakJwtAuthenticationFilter(String cibsevenApplicationPath) {
        if (StringUtils.hasLength(cibsevenApplicationPath)) {
            this.cibsevenApplicationPath = cibsevenApplicationPath;
        }
    }
    ...
    protected String extractEngineName(HttpServletRequest request) {
        String requestUri = request.getRequestURI().substring(cibsevenApplicationPath.length());
